...
if (isset($_POST['Nname'])&& isset($_POST['password'])) {
    ...
    if (empty($Nname)){
         header("Location: ngo.php?error=name is required");
         exit(); 
    }else if(empty($password)){
         header("Location: ngo.php?error=password is required");
         exit(); 
    }else{
        echo " ";
    }
...
if($conn->connect_error){
    ...
    $stmt = $conn->prepare("insert into register(resname,Nname,name,address,email,mobileno,password)values(?,?,?,?,?,?,?)");
    $stmt->bind_param("sssssis",$resname,$Nname,$name,$address,$email,$mobileno,$password);
    $stmt->execute();
    print("\n ");
    echo "<center></center>";
    $stmt->close();
